...
import { getAll, remove, status } from '../../api/RestaurantEndpoints'
...
export default function RestaurantsScreen ({ navigation, route }) {
  ...
  const renderRestaurant = ({ item }) => {
    return (
      ...
      <View style={[{ flex: 1, flexDirection: 'row', justifyContent: 'center' }]}>
        <MaterialCommunityIcons name='lock-clock' color={'white'} size={20}/>
              <TextRegular textStyle={styles.text}>
              online
              </TextRegular>
          </View>
...
<MaterialCommunityIcons name='lock-clock' color={'white'} size={20}/>
  ...
  const changeRestaurantStatus = async (item) => {
    try {
      await status(item.id)
      await fetchRestaurants()
      setRestaurantStatus(null)

      showMessage({
        message: `Restaurant ${item.name} could be change the status succesfully`,
        type: 'success',
